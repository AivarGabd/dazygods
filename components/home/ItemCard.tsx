...
const ItemCard = (item: Item) => {
    ...
    return (
        <Link href={`/item?id=${id}`} key={id}>
            <Card className="w-[200px] bg-[#f5f5f5]">
                ...
                <CardBody className="flex flex-col gap-2">
                    <h2 className="leading-none font-medium text-gray-500 text-sm">{item.title}</h2>
                ...
                <CardFooter className="flex flex-col gap-2">
                    <div className="flex gap-1 justify-center w-full">
                        <Button color='success' startContent={<ShoppingCart />} size="sm" className="w-full" >
                            В корзину
                        </Button>
